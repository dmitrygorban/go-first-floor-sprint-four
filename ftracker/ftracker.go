...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	averageSpeedMps := meanSpeed(action, duration) * kmhInMsec
	speedSquared := math.Pow(averageSpeedMps, 2)
	return ((walkingCaloriesWeightMultiplier*weight + (speedSquared/heightInMeters)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lenghtPool, countPool int, duration, weight float64) float64 {
	averageSwimSpeed := swimmingMeanSpeed(lenghtPool, countPool, duration)
